...
### git冲突合并
1. 设置git的冲突合并工具为vscode
```bash
# 设置差异比较工具为 VS Code
# 设置合并工具为 VS Code
# 配置 VS Code 作为差异工具的命令
# 配置 VS Code 作为合并工具的命令
git config --global diff.tool vscode
git config --global merge.tool vscode
git config --global difftool.vscode.cmd 'code --wait --diff $LOCAL $REMOTE'
# 使用--merge参数 启用vscode的3-way-merge合并
git config --global mergetool.vscode.cmd 'code --wait --merge $LOCAL $REMOTE $BASE $MERGED'
# 可选：设置不自动备份合并后的文件
git config --global mergetool.keepBackup false
```
说明：使用--merge参数，参数顺序依次是-工作区文件，其它分支文件，共同祖先，最终合并结果
2. vscode需要在设置里面开启git.MergeEditor
依次点击，设置->设置>搜索【git.MergeEditor】，选中
- [x] Open the merge editor for files that are currently under conflict.

> [vscode三方合并](https://code.visualstudio.com/updates/v1_69#_3-way-merge-editor '点我访问')
